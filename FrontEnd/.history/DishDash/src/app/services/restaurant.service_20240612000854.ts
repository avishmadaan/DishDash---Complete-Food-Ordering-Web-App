...
export class RestaurantService implements OnInit {
  ...
  fetchByCityURL:string = `http://localhost:8085/api/v3/restaurantsbycity`
  ...
  fetchRestaurantByid(resId:string):Observable<restaurant> {
    let fetchByIdURL = `http://localhost:8085/api/v3/restaurantsbyid/${resId}`;
    return this.http.get<restaurant>(fetchByIdURL); 
  }

  fetchAllRestaurants():Observable<Array<restaurant>> {
    return this.http.get<Array<restaurant>>("http://localhost:8085/api/v3/allrestaurants")
  }
